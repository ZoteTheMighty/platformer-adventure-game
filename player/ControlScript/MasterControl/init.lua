...
-- Reduces vertical speed by half in order to allow variable jump height
function MasterControl:EndJump()
	local humanoid = self:GetHumanoid()
	if humanoid then
		local humanoidRoot = humanoid.RootPart
		local hv = humanoidRoot.Velocity
		if hv.Y > 0 then
			humanoidRoot.Velocity = Vector3.new(hv.X, hv.Y / 2, hv.Z)
		end
	end
end
